...
async function migrateDb() {
	let rows = await libToEs6.dbQuery("SELECT name FROM sqlite_master WHERE type='table' AND name IN (?)", [['channels', 'address_pass', 'peer_addresses']]);
	if (rows.length === 3) {
		return true
	} else {
		...
		if (arrTableNames.indexOf('peer_addresses') === -1) {
			await libToEs6.dbQuery("CREATE TABLE peer_addresses(address CHAR(32) NOT NULL, device_address CHAR(33) NOT NULL,\n\
