...
class Channel {
	...
	async cb_new_joint(objJoint) {
		if (this.step !== 'await_createChannel') return;
	...
	async sendInputsAndAddresses() {
		this.addresses = await libAddress.getNonEmptyAddressesInWallet(this.walletId);
		let objPick = await Channel.pickDivisibleCoinsForAmount(db, {asset: null}, this.addresses, this.myAmount + constants.FEES_FOR_CREATE_CHANNEL);
	...
	async createChannel(objMessage) {
		this.addresses = await libAddress.getNonEmptyAddressesInWallet(this.walletId);
		let objPick = await Channel.pickDivisibleCoinsForAmount(db, {asset: null}, this.addresses, this.myAmount + constants.FEES_FOR_CREATE_CHANNEL, true);
		if (!objPick) {
		...
		return new Promise(((resolve, reject) => {
			...
			opts.outputs = [{
				...
				amount: objMessage.total_input - this.peerAmount - constants.FEES_FOR_CREATE_CHANNEL
			}, {
	...
	async getInputs(fromAddress) {
		...
		this.addresses = await libAddress.getNonEmptyAddressesInWallet(this.walletId);
		let objPick2 = await Channel.pickDivisibleCoinsForAmount(db, {asset: null}, this.addresses, constants.FEES_FOR_GET_INPUTS, true);
