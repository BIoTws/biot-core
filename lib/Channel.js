...
class Channel {
	...
	cb_new_joint(objJoint) {
		...
		if (outputs.findIndex(el => el.address === this.channelAddress) !== -1 &&
			this.step !== 'close' &&
			outputs.findIndex(el => el.address === this.myAddress) !== -1 &&
			outputs.findIndex(el => el.address === this.peerAddress) !== -1) {
			...
			if (objMessage.app === 'BIoT' && objMessage.id === this.id) {
				if (objMessage.status && objMessage.status === 'reject') {
					...
					core.sendTechMessageToDevice(this.peerDeviceAddress, {
						...
						arrDefinition: objSharedAddress.arrDefinition,
						id: this.id
					});
	...
	async cb_signing_request(objAddress, top_address, objUnit, assocPrivatePayloads, from_address, signing_path) {
		...
		let myAddressOutput = outputs.find(output => output.address === this.myAddress || output.address === this.myClosingAddress);
		let peerAddressOutput = outputs.find(output => output.address === this.peerAddress || output.address === this.peerClosingAddress);
		if (channelOutput || peerSharedAddressOutput || (this.step === 'await_closing' && myAddressOutput && peerAddressOutput && outputs.length === 2)) {
			if (this.step === 'await_createChannel' && signing_path === 'r') {
				let check = ChannelUtils.checkPaymentToSharedAddress(this, outputs, await core.getAddressesInWallet(this.walletId));
				if (!check.status) {
	...
	async sendInputsAndAddresses() {
		this.addresses = await libAddress.getNonEmptyAddressesInWallet(this.walletId);
		let objPick = await Channel.pickDivisibleCoinsForAmount(db, {asset: null}, this.addresses, this.myAmount + 500);
	...
	async createChannel(objMessage) {
		this.addresses = await libAddress.getNonEmptyAddressesInWallet(this.walletId);
		let objPick = await Channel.pickDivisibleCoinsForAmount(db, {asset: null}, this.addresses, this.myAmount + 500, true);
		if (!objPick) {
	...
	signMyTransfer(newMyAmount, newPeerAmount, objMyContract) {
		return new Promise(((resolve, reject) => {
			this.getInputs(this.channelAddress).then(objInputs => {
				let opts = {};
				opts.paying_addresses = [this.channelAddress].concat(objInputs.myPayingAddresses);
	...
	closeChannel(objInputs, from_addresses, outputs, signers, pass, requiredSigningAddresses) {
		return new Promise((resolve, reject) => {
			let opts = {};
			opts.outputs = outputs;
			opts.paying_addresses = from_addresses;
			if (requiredSigningAddresses)
				opts.signing_addresses = [this.myAddress, this.peerAddress];
			...
			return new Promise((resolve, reject) => {
				this.getInputs(this.channelAddress).then(async (objInputs) => {
					...
					this.closeChannel(objInputs, [this.channelAddress],
						...
						[this.myDeviceAddress, this.peerDeviceAddress], null, true)
						.then(async (objJoint) => {
							objInputs.unlock();
							this.step = 'mutualClose';
							this.mutualClose = true;
