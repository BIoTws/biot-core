...
class Channel {
	...
	async cb_signing_request(objAddress, top_address, objUnit, assocPrivatePayloads, from_address, signing_path) {
		...
		let myAddressOutput = outputs.find(output => output.address === this.myAddress);
			...
			let arrDefinition = ['or', [
				['and', [
					...
					['has', {
						what: 'output',
						asset: 'base',
						address: this.peerClosingAddress,
						amount: peerAmount
					}]
				]],
				['and', [
	...
	async transfer(amount, message) {
		if (this.step === 'close' || this.step === 'mutualClose') return {status: 'error', text: 'Channel closed'};
	...
	signMyTransfer(newMyAmount, newPeerAmount, objMyContract) {
		return new Promise(((resolve, reject) => {
			this.getInputs(this.channelAddress).then(objInputs => {
				...
				opts.signing_addresses = [this.myClosingAddress, this.peerClosingAddress];
	...
	async getInputs(fromAddress) {
		...
		if (!objPick) throw new Error('Insufficient funds contract address: ' + fromAddress);
			...
			return new Promise((resolve, reject) => {
				this.getInputs(this.channelAddress).then(async (objInputs) => {
					...
					this.closeChannel(objInputs, [this.channelAddress],
						[
							{address: this.peerAddress, amount: this.peerAmount},
							{address: this.myAddress, amount: 0}
						],
						[this.myDeviceAddress, this.peerDeviceAddress], null, [this.myClosingAddress, this.peerClosingAddress])
						.then(async (objJoint) => {
							this.removeTimer();
							this.step = 'mutualClose';
							this.mutualClose = true;
	...
	async punishPeer(address, pass) {
		this.getInputs(address).then(async (objInputs) => {
			...
			return this.closeChannel(objInputs, [address],
				[{
					address: await core.createNewAddress(this.walletId),
					amount: 0
				}],
	...
	async checkClosingPaymentAndPunish(unit) {
		if (this.objJoint && this.step !== 'close') {
			...
			if (addressesFromOutputs.length === 1) {
				if (addressesFromOutputs[0] === this.myClosingAddress) {
					checkPassed = true;
