...
class Channel {
	...
	startListeningEvents() {
		...
		eventBus.on('new_my_transactions', arrUnits => {
			arrUnits.forEach(async unit => {
				...
				if (ChannelUtils.isClosingPayment(this, arrAddresses)) {
					this.checkClosingPaymentAndPunishment(unit).catch(console.error);
				} else {
					console.error('!ok');
				}
			});
			...
			if (objMessage) {
				if (objMessage.app === 'BIoT' && objMessage.id === this.id) {
					if (objMessage.status && objMessage.status === 'reject') {
						...
						await libWallet.addIfNotExistRemoteWallet(objMessage.objWalletInfo);
		...
		eventBus.on("signing_request", async (objAddress, top_address, objUnit, assocPrivatePayloads, from_address, signing_path) => {
			...
			if (this.step === 'await_createChannel' && signing_path === 'r') {
				...
				await this.updateInDb();
				let rowsPass = await libToEs6.dbQuery("SELECT pass FROM address_pass WHERE id = ? AND address = ?", [this.id, this.prevSharedAddress]);
				core.sendTechMessageToDevice(this.peerDeviceAddress, {
					step: 'pass',
					id: this.id,
	...
	async saveHash(address, arrDefinition) {
		...
		return libToEs6.dbQuery("INSERT " + db.getIgnore() + " INTO address_pass (id, address, hash) VALUES(?,?,?)", [this.id, address, hash]);
	}

	async saveMyPass(address, pass) {
		return libToEs6.dbQuery("INSERT " + db.getIgnore() + " INTO address_pass (id, address, pass) VALUES(?,?,?)", [this.id, address, pass]);
	}

	async savePass(address, pass) {
		return libToEs6.dbQuery("UPDATE address_pass SET pass = ? WHERE id = ? AND address = ?", [pass, this.id, address]);
	}
	...
	async sendInputsAndAddresses() {
		...
		let objWalletInfo = await libWallet.getWalletFromDB(this.walletId);
	...
	async transfer(amount, message) {
		let unlock = await new Promise(resolve => {mutex.lock(["biot_transfer"], function (unlock) {return resolve(unlock)});});
	...
	closingPayment(objInputs, from_addresses, outputs, signers, pass) {
		return new Promise((resolve, reject) => {
			let opts = {};
	...
	async checkPassHash(address, pass) {
		let rowsPass = await libToEs6.dbQuery("SELECT hash FROM address_pass WHERE id = ? AND address = ?", [this.id, address]);
		return (rowsPass[0].hash === crypto.createHash("sha256").update(pass, "utf8").digest("base64"));
	}

	async punishPayment(address, pass) {
		return this.closingPayment(null, [address, this.addresses[0]],
			[{
				address: this.addresses[0],
				amount: 0
			}],
	...
	async checkClosingPaymentAndPunishment(unit) {
		if (this.objJoint) {
			...
			console.error('addresses', arrAddresses, unit);
	...
	async punish(unit) {
		...
		let rows = await libToEs6.dbQuery("SELECT address, pass FROM address_pass WHERE id = ? AND address IN (?)", [this.id, arrAddresses]);
		if (rows.length === 1) {
