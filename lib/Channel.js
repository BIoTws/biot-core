...
class Channel {
	constructor(params) {
		...
		if (this.step === 'waiting_mci_for_checking_final_close') {
	...
	signMyTransfer(newMyAmount, newPeerAmount, objMyContract) {
		return new Promise(((resolve, reject) => {
			this.getInputs(this.channelAddress).then(objInputs => {
				if(!objInputs) throw new Error('Insufficient funds contract address: ' + this.channelAddress);
	...
	async startWaitingMCI(unit, iPeer = false) {
		...
		this.intervalMCI = setInterval(async () => {
			if (await Channel.checkAgeGreaterThan(unit, this.age + value)) {
				...
				if (this.step === 'waiting_mci_for_checking_final_close') {
					await this.checkFinalCloseAndClosingIfNot();
				} else {
	...
	async checkFinalCloseAndClosingIfNot() {
		console.error('checkFinalCloseAndClosingIfNot', this.waitingUnit);
		...
		if (rows2.length === 1) {
			console.error('i\'m checkFinalCloseAndClosingIfNot', this.waitingUnit);
	...
	async checkClosingPaymentAndPunish(unit) {
		...
		if (this.objJoint) {
			...
			if (checkPassed) {
				this.step = 'waiting_mci_for_checking_final_close';
