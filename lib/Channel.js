...
class Channel {
	...
	async sendInputsAndAddresses() {
		this.addresses = await libAddress.getNonEmptyAddressesInWallet(this.walletId);
		let objPick = await Channel.pickDivisibleCoinsForAmount(db, {asset: null}, this.addresses, this.myAmount + constants.FEES_FOR_CHANNEL_OPERATIONS
			+ constants.FEES_FOR_CREATE_CHANNEL);
	...
	closeOneSideFinal() {
		return new Promise((resolve, reject) => {
			this.getInputs(this.objMyContract.shared_address).then(async (objInputs) => {
				...
				objInputs.arrInputs = objInputs.arrInputs.concat(objPick2.arrInputs);
				objInputs.total_input += objPick2.total_input;

				this.closeChannel(objInputs, [this.objMyContract.shared_address].concat(myPayingAddresses),
					[{
						address: this.peerAddress,
	...
	async punishPeer(address, pass) {
		this.getInputs(address).then(async (objInputs) => {
			...
			objInputs.arrInputs = objInputs.arrInputs.concat(objPick2.arrInputs);
			objInputs.total_input += objPick2.total_input;
			return this.closeChannel(objInputs, [address].concat(myPayingAddresses),
				[{
					address: this.myClosingAddress,
					amount: 0
				}],
