...
async function getAddressesFromDb(addresses) {
	...
	return await toEs6.dbQuery("SELECT address, wallet, is_change, address_index, definition \n\
		FROM my_addresses WHERE address IN (?)", [addresses])
}

async function addIfNotExistRemoteAddresses(arrAddressesRows) {
	arrAddressesRows.forEach(async (row) => {
		await toEs6.dbQuery("INSERT " + db.getIgnore() + " INTO my_addresses (address, wallet, is_change, address_index, definition) \n\
			VALUES (?,?,?,?,?)",
			[row.address, row.wallet, row.is_change, row.address_index, row.definition]);
	});
