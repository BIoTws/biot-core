...
class AutoRenewableChannel {
				...
				if (objMessage.app === 'BIoT') {
					if (objMessage.step === 'get1Contract') {
						...
						if (this.currentChannel === null) {
							...
							this.events.emit('firstNewChannel', objMessage);
	...
	closeMutually(){
		this.currentChannel.closeMutually();
		this.nextChannel.closeMutually();
	}
	
	closeOneSide(){
		this.currentChannel.closeOneSide();
		this.nextChannel.closeOneSide();
	}
