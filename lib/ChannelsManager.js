...
class ChannelsManager {
	...
	recoveryChannel(row) {
		...
		return new Channel(this.walletId, device.getMyDeviceAddress(), row.peerDeviceAddress, row.peerAddress,
			row.myAmount, row.peerAmount, row.age, row.id, {
				...
				objJoint: ChannelsManager.parseJSON(row.objJoint)
			});
	}
	...
	getNewChannel(objInfo) {
		...
		return new Channel(this.walletId, device.getMyDeviceAddress(), objInfo.peerDeviceAddress, objInfo.myAddress,
			objInfo.peerAmount, objInfo.myAmount, objInfo.age, objInfo.id);
	}

	reject(objInfo) {
		console.error('sent Reject', objInfo);
		core.sendTechMessageToDevice(objInfo.peerDeviceAddress, {
			id: objInfo.id,
			step: objInfo.step,
			status: 'reject'
		});
