...
const ecdsaSig = require('ocore/signature');
...
if (!isCordova) {
	desktopApp = require('ocore/desktop_app' + '');
	appDataDir = desktopApp.getAppDataDir();
	KEYS_FILENAME = appDataDir + '/' + (conf.KEYS_FILENAME || 'keys.json');
} else {
	KEYS_FILENAME = _fs.getDatabaseDirPath() + '/keys';
}
...
async function keysFileExists() {
	return await toEs6.fsAccess(KEYS_FILENAME);
}
...
async function readKeys(passphrase) {
	...
	if (!isAccess) {
		...
		return new Promise((resolve, reject) => {
			_fs.readFile(KEYS_FILENAME, async (err, data) => {
				if (err) {
					return reject(err);
				}
				...
				if (!rows.length) {
					...
					let walletId = await wallet.createWallet(xPrivKey, 0);
					if (global.window && window.cordova) {
						let assocWalletToName = {};
						assocWalletToName[walletId] = 'Main wallet';
...
async function writeKeys(mnemonic_phrase, deviceTempPrivKey, devicePrevTempPrivKey) {
	...
	if (isCordova) {
		await new Promise(resolve => {
			_fs.cordovaWriteFile(_fs.getParentDirPath(), _fs.getDatabaseDirName(), 'keys', JSON.stringify(keys, null, '\t'), resolve);
		})
	} else {
...
function findAddress(address, signing_path, callbacks, fallback_remote_device_address) {
	...
	db.query(
		...
		rows => {
			...
			db.query("SELECT device_address FROM biot_peer_addresses WHERE address = ?", [address], rows2 => {
				if (rows2.length === 1) {
...
function getLocalSigner(opts, arrSigningDeviceAddresses, signWithLocalPrivateKey) {
	...
	signer.sign = (objUnsignedUnit, assocPrivatePayloads, address, signing_path, handleSignature) => {
		const authors = _.cloneDeep(objUnsignedUnit.authors);
		...
		findAddress(address, signing_path, {
			...
			ifRemote: function (device_address) {
				...
				let newObjUnsignedUnit = _.cloneDeep(objUnsignedUnit);
				newObjUnsignedUnit.authors = authors;
				walletGeneral.sendOfferToSign(device_address, address, signing_path, newObjUnsignedUnit, assocPrivatePayloads);
