...
exports.checkPaymentToChannelContract = (channel, outputs, myAddresses) => {
	...
	if (channelOutput.amount !== (channel.myAmount + channel.peerAmount + (constants.FEES_FOR_CHANNEL_OPERATIONS * 2))) {
		return {status: false, code: 2};
	}
	...
	if (myOutput.amount !== (channel.total_input - channel.myAmount - constants.FEES_FOR_CREATE_CHANNEL)) {
		return {status: false, code: 4};
	}
