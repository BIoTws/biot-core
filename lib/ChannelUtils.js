...
exports.check1Contract = (channel, shared_address, arrDefinition, transferAmount = 0, isRecipient) => {
	...
	if (arrDefinition[1][0][1][0][1][0][1] !== channel.peerUnilateralAddress) {
		return {status: false, code: 1};
	}

	if (arrDefinition[1][0][1][0][1][1][1] !== channel.myUnilateralAddress) {
		return {status: false, code: 2};
	}
	...
	if (arrDefinition[1][0][1][2][1].address !== channel.peerDestinationAddress) {
		return {status: false, code: 4};
	}

	if (arrDefinition[1][0][1][3][1].address !== channel.myDestinationAddress) {
		return {status: false, code: 5};
	}
	...
	if (arrDefinition[1][1][1][0][1] !== channel.myUnilateralAddress) {
...
exports.checkClosingPayment = (channel, outputs) => {
	...
	let myOutput = outputs.find(output => {
		return output.address === channel.myDestinationAddress;
	});

	let peerOutput = outputs.find(output => {
		return output.address === channel.peerDestinationAddress;
	});
